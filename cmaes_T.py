...
# ---------------------------------------------------------------
# @profile
def cmaes_T(fitnessfunc=frosenbrock, N=10, initX=None, initsgm=0.3,
    ...
    pc = zeros((1, N));
    ps = zeros((1, N));  # evolution paths for C and sigma
    ...
    if save is True:
        cacheL = 50
        xmean_cache = zeros((cacheL, N))

    while counteval < stopeval:
        # Generate and evaluate lambda_ offspring
        for k in range(lambda_):
            arx[k:k + 1, :] = xmean + sigma * (D * randn(1, N)) @ B.T  # m + sig * Normal(0,C)
        ...
        xold = xmean
        xmean = weights @ arx[arindex[0:mu], :]   # recombination, new mean value
        # sliced mat * vect = mean col of feature

        # Cumulation: Update evolution paths
        ps = (1 - cs) * ps + sqrt(cs * (2 - cs) * mueff) * (xmean - xold) / sigma @ invsqrtC
        ...
        C = ((1 - c1 - cmu) * C  # regard old matrix
             ...
             + cmu * artmp.T @ diag(weights.flat) @ artmp)  # plus rank mu update
        ...
        if counteval - eigeneval > lambda_ / (c1 + cmu) / N / 10:  # to achieve O(N^2)
            eigeneval = counteval;
            t1 = time()
            C = (C + C.T) / 2  # np.triu(C) + np.triu(C, 1).T;  # enforce symmetry
...
cmaes_T(N=1000, initX=[-1] * 1000, initsgm=1, save=False, savemrk='tmp')
